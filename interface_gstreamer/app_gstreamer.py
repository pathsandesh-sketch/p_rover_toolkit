...
# Swapped ffmpeg TCP for ultra-low latency GStreamer UDP pipeline listening on port 8282
STREAM_URL = (
    "udpsrc port=8282 caps=\"application/x-rtp, media=(string)video, clock-rate=(int)90000, "
    "encoding-name=(string)H264, payload=(int)96\" ! rtpjitterbuffer latency=0 ! "
    "rtph264depay ! avdec_h264 ! videoconvert ! appsink sync=false"
)
print("\n[STARTING UPHUB] Opening port 8282 via GStreamer. Waiting for Raspberry Pi video...")
